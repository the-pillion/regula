...
- Expanded accountability registers in `foundation.json`: **processor register = 8 verified third parties** (stripe, fastgeo, tomtom, firebase, twilio, cloudflare + ionos & hetzner VPS hosts). Each vendor's transfer mechanism researched + verified 2026-06-08 (Stripe/Cloudflare/Twilio/Firebase EU-US DPF + SCCs; FastGeo/TomTom/IONOS/Hetzner intra-EEA); DPA URLs in `notes`; `dpa_status` still pending (execution unconfirmed). **Owner-confirmed removals: Zitadel (self-hosted on EU VPS → not a processor), Checkr (not used), Neon (not used).** Retention policies (added account-data, ride-records, payment-records, marketing-consent), Article 30 RoPA (added safety/SOS, support/disputes, communications/marketing, fraud-prevention, **data-protection-governance = DPO function**; auth + onboarding activities updated for self-hosted identity + internal PrefactGuard screening), and DPIA (added automated-eligibility-decisions). DPO designation flows into docs via company JSON; no new DB table introduced.
